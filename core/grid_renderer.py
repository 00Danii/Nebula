...
class GridRenderer:
    ...
    def render(
        ...
        font_main = self._font_manager.load(self._font_path, 14)
        font_small = self._font_manager.load(self._font_path, 11)

        if image_rect:
            ix, iy, iw, ih = image_rect
            self._draw_margin_ticks(draw, ix, iy, iw, ih, grid_color, font_small, num_lines)
    ...
    def _draw_margin_ticks(self, draw, ix, iy, iw, ih, color, font, num_lines):
        ...
        for i in range(1, num_lines):
            ...
            draw.line([(tx, iy - tick_len), (tx, iy)], fill=color, width=1)
            draw.line([(tx, iy + ih), (tx, iy + ih + tick_len)], fill=color, width=1)
            draw.line([(ix - tick_len, ty), (ix, ty)], fill=color, width=1)
            draw.line([(ix + iw, ty), (ix + iw + tick_len, ty)], fill=color, width=1)
